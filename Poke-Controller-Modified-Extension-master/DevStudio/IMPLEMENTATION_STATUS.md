...
- Source Edit now has a `関数登録` right-side tab immediately after Completion. It supports searchable multi-selection, bulk function-name previews (find/replace/prefix/suffix or direct override), safe definition/reference renaming, one-fragment-per-function registration, overwrite protection, and direct navigation to Sample functions edit mode.
